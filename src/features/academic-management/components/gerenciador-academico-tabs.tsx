...
export function GerenciadorAcademicoTabs({
  ...
  // Abas do Admin: apenas Unidades Académicas e Cursos (Turmas geridas no /coordenador/turmas)
  const [abaAtiva, setAbaAtiva] = useState<"unidades" | "cursos">("unidades");
  ...
  const confirmarEliminacao = () => {
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        setNotificacao({
          tipo: "sucesso",
          texto: `${modalEliminar.tipo === "unidade" ? "Unidade" : "Curso"} eliminada com sucesso!`,
        });
  ...
  return (
    <div className="space-y-6">
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100 text-sm">
                    {cursosFiltrados.map((c) => {
                      const totalTurmas = c._count?.turmas ?? 0;
      ...
      {/* MODAL 3: CONFIRMAÇÃO DE ELIMINAÇÃO */}
      {modalEliminar && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-xs animate-in fade-in duration-150">
          <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl border border-slate-100">
            <div className="flex items-center gap-3 text-rose-600">
              ...
              <h3 className="text-lg font-black text-slate-900">
                Eliminar {modalEliminar.tipo === "unidade" ? "Unidade Académica" : "Curso"}
              </h3>
            </div>
