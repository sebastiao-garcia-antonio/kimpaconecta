...
export default async function PaginaMensagens({ searchParams }: { searchParams: Promise<{ conversa?: string }> }) {
  ...
  return <div className="space-y-8"><PaginaSecao papel={papel} titulo="Mensagens" descricao="Converse em privado com membros ativos da comunidade académica." indicadores={[{ titulo: "Conversas", valor: String(conversas.length), observacao: "Canais privados" }, { titulo: "Contactos", valor: String(contactos.length), observacao: "Utilizadores disponíveis" }, { titulo: "Sincronização", valor: "10 s", observacao: "Atualização automática" }]} resumos={[{ titulo: "Comunicação segura", descricao: "Apenas participantes da conversa podem consultar e enviar mensagens.", estado: "Ativa" }]} /><div className="px-6 pb-8 lg:px-8"><ChatDiretoClient idUsuario={idUsuario} contactos={JSON.parse(JSON.stringify(contactos))} conversas={JSON.parse(JSON.stringify(conversasSerializadas))} conversaAtiva={conversaAtiva ? JSON.parse(JSON.stringify(conversaAtiva)) : null} /></div></div>;
}