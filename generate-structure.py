...
base = r"c:\kimpaConeta\src"

print("--- RUNNING PYTHON ENTERPRISE SCAFFOLD GENERATOR ---")

# 1. CORE STYLES & UTILS
write_file(os.path.join(base, "styles", "globals.css"), """
@tailwind base;
@tailwind components;
@tailwind utilities;
...
write_file(os.path.join(base, "lib", "prisma.ts"), """
...
write_file(os.path.join(base, "lib", "socket.ts"), """
...
write_file(os.path.join(base, "lib", "utils.ts"), """
...
# 2. GLOBAL PROVIDERS
write_file(os.path.join(base, "providers", "query-provider.tsx"), """
"use client";
...
write_file(os.path.join(base, "providers", "theme-provider.tsx"), """
"use client";
...
# 3. AUTH MÓDULO (NextAuth.js v5 / Auth.js)
write_file(os.path.join(base, "features", "auth", "schemas.ts"), """
import { z } from "zod";
...
write_file(os.path.join(base, "features", "auth", "repositories", "auth.repository.ts"), """
...
write_file(os.path.join(base, "features", "auth", "actions.ts"), """
"use server";
...
write_file(os.path.join(base, "features", "auth", "auth.config.ts"), """
...
# 4. CHAT E COMUNICAÇÃO MODULE (Messaging)
write_file(os.path.join(base, "features", "messaging", "repositories", "messaging.repository.ts"), """
import { prisma } from "@/lib/prisma";
...
write_file(os.path.join(base, "features", "messaging", "actions.ts"), """
"use server";
...
# 5. AVALIAÇÕES E PROCTORING MODULE (Assessments)
write_file(os.path.join(base, "features", "assessments", "repositories", "assessments.repository.ts"), """
import { prisma } from "@/lib/prisma";
...
write_file(os.path.join(base, "features", "assessments", "hooks", "use-proctoring.ts"), """
"use client";
...
# 6. LOCAL UPLOADS MODULE (Files)
write_file(os.path.join(base, "features", "files", "services", "upload.service.ts"), """
...
# 7. ROUTING AND APP ROUTER (App pages)
write_file(os.path.join(base, "app", "layout.tsx"), """
...
write_file(os.path.join(base, "app", "page.tsx"), """
...
write_file(os.path.join(base, "middleware.ts"), """
...
write_file(os.path.join(base, "hooks", "use-toast.ts"), """
...
write_file(os.path.join(base, "app", "(auth)", "login", "page.tsx"), """
import Link from "next/link";
...
write_file(os.path.join(base, "app", "(auth)", "registro", "page.tsx"), """
import Link from "next/link";
import { GraduationCap } from "lucide-react";
...
dashboards = ["admin", "coordenador", "professor", "estudante"]
...
""".replace("[DASHBOARD]", db)
    write_file(os.path.join(base, "app", "(dashboard)", db, "page.tsx"), content)

write_file(os.path.join(base, "app", "api", "proctoring", "log", "route.ts"), """
import { NextResponse } from "next/server";
...
features = ["academic", "users", "notifications", "networking", "analytics", "files"]
for feat in features:
    ensure_dir(os.path.join(base, "features", feat, "components"))
    ensure_dir(os.path.join(base, "features", feat, "hooks"))
    ensure_dir(os.path.join(base, "features", feat, "services"))
    
    repo_content = """
import { prisma } from "@/lib/prisma";

export class [FEAT_CAP]Repository {
  static async getEntity(id: number) {
    return { id, status: "ok" };
  }
}
""".replace("[FEAT_CAP]", feat.capitalize())
    write_file(os.path.join(base, "features", feat, "repositories", f"{feat}.repository.ts"), repo_content)
    
    action_content = """
"use server";

import { [FEAT_CAP]Repository } from "./repositories/[FEAT].repository";

export async function processAction(id: number) {
  try {
    const data = await [FEAT_CAP]Repository.getEntity(id);
    return { success: true, data };
  } catch (error) {
    return { error: "Action error" };
  }
}
""".replace("[FEAT_CAP]", feat.capitalize()).replace("[FEAT]", feat)
    write_file(os.path.join(base, "features", feat, "actions.ts"), action_content)
